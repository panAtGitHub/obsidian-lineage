...
describe('syncDayPlanTitlesInMarkdown', () => {
    ...
    it('supports batch sync by sections', () => {
        const markdown =
            '---\n' +
            'mandala: true\n' +
            'mandala_plan:\n' +
        ...
        const batches = createDayPlanTitleSyncBatches(markdown, 2);
        expect(batches).not.toBeNull();
        expect(batches?.total).toBe(3);

        let working = markdown;
        for (const batch of batches?.batches ?? []) {
            working = syncDayPlanTitlesBySections(working, batch).markdown;
        }
        expect(working).toContain('<!--section: 1.1-->\n### 接收太阳的能量，开心一天\nbody');
        expect(working).toContain('<!--section: 1.2-->\n### 09-12\ntext');
        expect(working).toContain('<!--section: 1.3-->\n### 12-14');
    });
});
